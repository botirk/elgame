...
class Form extends AbstractGame<{ words: WordWithImage[], setup: FormGameSetup }, ReturnType<typeof calcForm>, ReturnType<typeof calcFormPos>, EndGameStats> {
  private _curForm: Form;
  private _wonForms: Form[] = [];
  private _lostForms: Form[] = [];
  private score = {
    total: 0, health: 3,
    required: 
  ...
  private showNextForm() {
    ...
    this._curForm = new Form(this.init, this.content.setup, nextAnswer, nextFalseAnswers, function(word) {
      if (word !== nextAnswer) {
